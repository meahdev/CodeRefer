...
class CellDimensions {
  static const CellDimensions base = CellDimensions.fixed(
    ...
    stickyLegendWidth: 80.0,
    headerViewHeight: 65.0,
  );

  /// Same dimensions for each cell.
  const CellDimensions.uniform({
    required double contentCellWidthValue,
    required double stickyLegendWidth,
  }) : this.fixed(
          contentCellWidth: contentCellWidthValue,
          contentCellHeight: 100.0,
  ...
  const CellDimensions.fixed({
    ...
    required this.headerViewHeight,
  ...
  Size contentSize(int i, int j) {
    try {
      ...
      print('exception :contentSize(int i, int j) $e');
      final width = base.contentCellWidth;
      final height = base.contentCellHeight;
  ...
  double stickyWidth(int i) {
    try {
      return (columnWidths != null ? columnWidths![i] : contentCellWidth) ??
          base.contentCellWidth!;
    } catch (e) {
      print('exception : stickyWidth(int i) $e');
      return base.contentCellWidth!;
    }
  }
